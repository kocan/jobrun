        ...
        <ScrollView style={styles.container} contentContainerStyle={styles.content}>
          {editing ? (
            ...
            <>
              ...
              {form.status === 'draft' && (
                <Pressable accessibilityRole="button" accessibilityLabel="Tap to activate action" style={[styles.actionBtn, { backgroundColor: '#3B82F6' }]} onPress={() => handleStatusChange('sent')}>
                  <Text style={styles.actionBtnText}>✉ Mark as Sent</Text>
                </Pressable>
              )}
              {form.status === 'sent' && (
                <>
                  <Pressable accessibilityRole="button" accessibilityLabel="Tap to activate action" style={[styles.actionBtn, { backgroundColor: '#10B981' }]} onPress={() => handleStatusChange('accepted')}>
                    <Text style={styles.actionBtnText}>✓ Mark Accepted</Text>
                  </Pressable>
                  <Pressable accessibilityRole="button" accessibilityLabel="Tap to activate action" style={[styles.actionBtn, { backgroundColor: '#EF4444' }]} onPress={() => handleStatusChange('declined')}>
                    <Text style={styles.actionBtnText}>✕ Mark Declined</Text>
                  </Pressable>
                </>
              )}
              {form.status === 'accepted' && !getEstimateById(id!)?.jobId && (
                <Pressable accessibilityRole="button" accessibilityLabel="Tap to activate action" style={[styles.actionBtn, { backgroundColor: '#EA580C' }]} onPress={handleConvertToJob}>
                  <Text style={styles.actionBtnText}>🔧 Convert to Job</Text>
                </Pressable>
              )}
              {form.status === 'accepted' && (
                <Pressable accessibilityRole="button" accessibilityLabel="Tap to activate action" style={[styles.actionBtn, { backgroundColor: '#7C3AED' }]} onPress={() => router.push({ pathname: '/invoice/[id]', params: { id: 'new', fromEstimate: id } })}>
                  <Text style={styles.actionBtnText}>📄 Create Invoice</Text>
                </Pressable>
              )}
              {(form.status === 'declined' || form.status === 'expired') && (
                <Pressable accessibilityRole="button" accessibilityLabel="Tap to activate action" style={[styles.actionBtn, { backgroundColor: '#6B7280' }]} onPress={() => handleStatusChange('draft')}>
