        ...
        <ScrollView style={styles.container} contentContainerStyle={styles.content}>
          {editing ? (
            <>
              ...
              <Pressable accessibilityRole="button" accessibilityLabel="Tap to activate action" style={styles.cancelBtn} onPress={() => isNew ? router.back() : setEditing(false)}>
            ...
            <>
              ...
              {form.status === 'draft' && (
                <Pressable accessibilityRole="button" accessibilityLabel="Tap to activate action" style={[styles.actionBtn, { backgroundColor: '#3B82F6' }]} onPress={async () => {
                  await updateInvoice(id!, { status: 'sent' });
                  setForm((f) => ({ ...f, status: 'sent' }));
                }}>
              ...
              {(form.status === 'sent' || form.status === 'viewed' || form.status === 'overdue') && (
                <Pressable accessibilityRole="button" accessibilityLabel="Tap to activate action" style={[styles.actionBtn, { backgroundColor: '#10B981' }]} onPress={handleMarkAsPaid}>
                  <Text style={styles.actionBtnText}>💰 Mark as Paid</Text>
                </Pressable>
              )}
              {(form.status === 'sent' || form.status === 'viewed') && (
                <Pressable accessibilityRole="button" accessibilityLabel="Tap to activate action" style={[styles.actionBtn, { backgroundColor: '#EF4444' }]} onPress={async () => {
              ...
              {form.status !== 'paid' && form.status !== 'cancelled' && (
                <Pressable accessibilityRole="button" accessibilityLabel="Tap to activate action" style={[styles.actionBtn, { backgroundColor: '#9CA3AF' }]} onPress={async () => {
              ...
              {form.status === 'cancelled' && (
                <Pressable accessibilityRole="button" accessibilityLabel="Tap to activate action" style={[styles.actionBtn, { backgroundColor: '#6B7280' }]} onPress={async () => {
